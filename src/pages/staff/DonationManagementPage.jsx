...
const FilterBar = React.memo(
  ({ selectedDate, onDateChange, onSearch, onReset }) => (
...
export default function DonationManagementPage() {
  ...
  const fetchBloodTypes = useCallback(async () => {
    ...
    try {
      const response = await axiosClient.get("/blood-types");
      setBloodTypes(response.data);
    } catch (error) {
      console.error("Không thể tải danh sách nhóm máu.", error);
    }
  }, [bloodTypes.length]);
  ...
  const fetchAllAppointments = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const response = await axiosClient.get("/staff/appointments");
  ...
  const handleSearchByDate = useCallback(async () => {
    ...
    try {
      ...
      const response = await axiosClient.get("/staff/appointments/by-date", {
  ...
  }, [notificationContent.type, handleCloseAllModals, fetchAllAppointments]);
  ...
  const handleScreeningSubmit = useCallback(
    async (formData) => {
      ...
      try {
        ...
        await axiosClient.post("/staff/user-profile", userProfilePayload);
        const screeningPayload = { passed, remarks };
        await axiosClient.post(
          `/staff/appointments/${currentAppointment.appointmentId}/screening`,
          screeningPayload
        );
  ...
  const handleRecordDonationSubmit = useCallback(
    async (formData) => {
      ...
      setIsSubmitting(true);
      try {
        await axiosClient.post("/staff/donation-history/record", formData);
  ...
  const handleOpenModal = useCallback(
    async (modalType, appointment) => {
      ...
      switch (modalType) {
        ...
        case "history":
          try {
            const response = await axiosClient.get(
              `/staff/donation-history/user/${appointment.user.userId}`
            );
        ...
        case "recordDonation":
        case "profile": {
          // thêm dấu ngoặc {} ở đây để bao bọc khai báo biến
          let userProfile = null;
          try {
            const profileRes = await axiosClient.get(
              `/staff/user-profile/${appointment.user.userId}`
            );
  ...
  const renderContent = () => {
    if (loading) {
      return (
        <p className="text-center text-gray-500 py-20">Đang tải dữ liệu...</p>
      );
    }
    if (error) {
      return (
