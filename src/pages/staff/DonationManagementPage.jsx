...
// Import thư viện DatePicker và CSS của nó (CSS này cần cho phần lịch bung ra)
import DatePicker from 'react-datepicker';
import 'react-datepicker/dist/react-datepicker.css';

// --- CÁC HÀM TIỆN ÍCH ---

// Định dạng ngày từ "YYYY-MM-DD" thành "DD/MM/YYYY" để hiển thị
...
// Định dạng đối tượng Date thành chuỗi "YYYY-MM-DD" để gửi cho API
const formatDateForApi = (dateObject) => {
  if (!dateObject) return '';
  // Chuyển đổi để tránh lỗi múi giờ
  const offset = dateObject.getTimezoneOffset();
  const adjustedDate = new Date(dateObject.getTime() - (offset*60*1000));
  return adjustedDate.toISOString().split('T')[0];
};

// --- COMPONENT TÙY CHỈNH CHO Ô CHỌN NGÀY ---
// Component này được style bằng Tailwind và được truyền vào DatePicker
...
export default function DonationManagementPage() {
  ...
  // Lấy TẤT CẢ các cuộc hẹn khi component được tải lần đầu
  ...
  // Map trạng thái sang màu sắc của Tailwind
  const statusStyles = {
                ...
                <ul className="bg-white rounded-b-lg shadow-md divide-y divide-gray-200">
                  {groupedAppointments[date].map(app => (
                    <li key={app.appointmentId} className="px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                      ...
                      <Link 
                        to={`/staff/donors/${app.appointmentId}`} 
                        className="ml-4 flex-shrink-0 px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors"
                      >
                        Xem chi tiết
                      </Link>
                    </li>
